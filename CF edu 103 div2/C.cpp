...
int main()
{
    ...
    while(t--)
    {
        ...
        for(int i=1; i<n; i++)
        {
            ...
            dp[i][0]=max(dp[i-1][0], dp[i-1][1])+pro[i]+2;
            dp[i][1]=max(dp[i-1][1]+2, pro[i]);
        }

        cout<<max(max(dp[n-1][0], dp[n-1][1]), ans)<<endl;
...
//            if(temp!=0)
//            {
//                temp-=abs(brr[i-1]-crr[i-1]);
//            }
//            temp+=(abs(brr[i]-crr[i])+1+arr[i]);
//            ans=max(ans,temp);
//        }
//        cout<<ans<<endl;
