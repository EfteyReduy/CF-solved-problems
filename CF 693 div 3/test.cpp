...
int main()
{
    ...
    while(t--)
    {
        int n;
        cin>>n;
        int ans=1000000001, temp=0, idx;
        pair<int, int>p[n], pp, pa={0,1000000001};
        ...
        for(int i=0; i<n; i++)
        {
            ...
            temp=a;
            s.insert({{p[i]}, i});
            if(temp<ans or (temp==ans and pa.second>b))
            {
                pa={a,b};
                ans=min(ans, temp);
                idx=i;
        ...
        for(int i=0; i<n; i++)
        {
            auto it=s.lower_bound({{p[i].first, p[i].second}, 0});
            auto xx=s.lower_bound({{0, 0},0});
            auto x=(*xx);
