...
main()
{
    ...
    while(t--)
    {
        ...
        for(int i=1; i<n; i++)
        {
            dp[1][i]=tsum-((arr[i]*brr[i])+(arr[i-1]*brr[i-1]))+(arr[i]*brr[i-1])+(arr[i-1]*brr[i]);
            ans=max(ans, dp[1][i]);
        }
        for(int i=2; i<n; i++)
        {
            for(int j=i; j<n; j++)
            {
                dp[i][j]=dp[i-2][j-1]+(arr[j]*brr[j-i])+(arr[j-i]*brr[j])-(arr[j]*brr[j])-(arr[j-i]*brr[j-i]);
                ans=max(ans,dp[i][j]);
            }
        }
//        for(int i=0; i<n; i++)
