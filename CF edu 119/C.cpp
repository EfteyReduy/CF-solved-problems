...
main()
{
    ...
    while(t--)
    {
        ...
        {
            ll pos = k*vec[i];
            if(log10(pre)+log10(pos)>=18)
            {
                for(ll j=1; j<=pos; j++)
                {
                    if((j+1)*pre>x){
                        tot.push_back((j+1)*pre);
                        break;
                    }
        ...
        ll tsz=tot.size(), vsz=vec.size();
        tot.push_back(1);
        vector<ll>taken;
        for(int i=0; i<tot.size()-1; i++)
        {
            ll j;
            for(j=0; j<=vec[vsz-tsz+i]*k; j++)
            {
                if(((j+1)*tot[i+1])>x)
                {
                    break;
        ...
        for(int i=n-1; i>=0; i--)
        {
            if(s[i]=='a')
            {
                ...
                i++;
                for(int j=0; idx>=0 and j<taken[idx]; j++)
                {
                    ans+='b';
                }
                idx--;
