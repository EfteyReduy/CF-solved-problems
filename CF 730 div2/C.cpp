...
void dfs(string st, int n)
{
    if(n>12)
    {
        double x=c, y=m, z=p, tmp=1;
        bool notc=true, notm=true, elemc=false, elemm=false;
        string ss="";
        for(int i=0; i<st.length(); i++)
        {
            if(st[i]=='c')
            {
                if((notc or x>0) and !elemc)
                {
                    ss+="c";
                    tmp*=x;
                    y+=(min(v, x)/2.000000);
                    ...
                    if(mp[ss]==0)
                    {
                        temp+=(z*(ss.size()+1.0000000)*tmp);
                        mp[ss]=1;
                        double added=(z*(ss.size()+1.0000000)*tmp);
                    }
                    x-=v;
                    notc=false;
                }
