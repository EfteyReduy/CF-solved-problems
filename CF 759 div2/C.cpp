...
main()
{
    ...
    while(t--)
    {
        ...
        for(int i=lft.size()-1; i>=0; i-=k)
        {
            totl+=(lft[i]*2);
        }

        for(int i=rit.size()-1; i>=0; i-=k)
        {
            totr+=(rit[i]*2);
        }

        ll disl=1e15, disr=1e15;

        if(lft.size()!=0)
        {
            disl=lft[lft.size()-1];
            for(int i=lft.size()-1-k; i>=0; i-=k)
            {
                disl+=(lft[i]*2);
            }
        }
        ...
        {
            disr=rit[rit.size()-1];
            for(int i=rit.size()-1-k; i>=0; i-=k)
            {
                disr+=(rit[i]*2);
            }
        }
        ll x=max((ll)0, totl-disl);
        ll y=max((ll)0, totr-disr);
        ll ans=(totl+totr)-max(x,y);
