...
main()
{
    ...
    while(t--)
    {
        ...
        {
            cout<<max(arr[0],arr[1])-min(arr[0], arr[1])<<endl;
            continue;
        }
        sort(arr, arr+n);
        ll ans=1e13;
        for(int x=0; x<n; x++)
        {
            ...
            while(1)
            {
                ...
                if(((arr[j]-arr[i])*mp[arr[i]]+(arr[j]-men)*mp[arr[j]])<(mex-arr[i])*mp[arr[i]]+(arr[j]-arr[i])*mp[arr[j]])
                {
                    if(j<n)
                    {
                ...
                else //if(mp[arr[i]]*(mex-arr[i])<(arr[j]-men)*mp[arr[j]])
                {
                    if(i>=0)
                    {
                        men=arr[i];
