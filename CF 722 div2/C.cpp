...
void dfs(int node, int par, int val)
{
    ...
    for(int i=0; i<g[node].size(); i++)
    {
        if(!vis[g[node][i]])
        {
            if(abs(val-ump[g[node][i]].first)>abs(val-ump[g[node][i]].second))
            {
                temp+=abs(val-ump[g[node][i]].first);
                db(temp);
                dfs(g[node][i], node, ump[g[node][i]].first);
            }
            else{
                cout<<"temp is "<<temp<<endl;
                temp+=abs(val-ump[g[node][i]].second);
                dfs(g[node][i], node, ump[g[node][i]].second);
            }
        }
    }
}
...
main()
{
    ...
    while(t--)
    {
        ...
        dfs(p[0].second, -1, ump[p[0].second].second);
        ll ans=temp;
        temp=0;
        dfs(p[0].second, -1, ump[p[0].second].first);
        cout<<max(ans, temp);
        for(int i=0; i<=n; i++)
        {
            g[i].clear();
